...
proc vTcl:splash {} {
    ...
    image create photo "title" -file [file join $vTcl(VTCL_HOME) images title.gif]
    wm overrideredirect .x 1
    label .x.l -image title -bd 1 -relief sunken -background black
    pack .x.l -side top -expand 1 -fill both
    entry .x.status -relief flat -background black -foreground white -textvar statusMsg -font "Monospace 8"
...
proc vTcl:load_widgets {} {
    ...
    foreach i $toload {
        ...
        if {[vTcl:load_lib $i]} {
            ...
            if {$tcl_platform(platform)!="windows" && $libname=="lib_tkvideo"} {
                continue
            }
...
proc vTcl:setup_gui {} {
    ...
    if {$vTcl(pr,font_fixed) == ""} {
        set vTcl(pr,font_fixed) {Monospace 10}
    }
    ...
    if {$tcl_platform(platform) == "unix"} {
        ...
        option add *vTcl*font {Monospace 12}
    ...
    option add *Font             {Monospace 10}
    #option add *Foreground       white
    #option add *Background       #202024
...
proc vTclWindow.vTcl {args} {
    ...
    if {$tcl_platform(platform) == "macintosh"} {
        ...
        regsub -all {[0-9]+x[0-9]+} $vTcl(geometry,.vTcl) "" vTcl(geometry,.vTcl)
        wm geometry .vTcl $vTcl(geometry,.vTcl)
    }
    
    wm protocol .vTcl WM_DELETE_WINDOW {vTcl:quit}
    set tmp $vTcl(gui,main).menu
    ttk::frame $tmp -relief flat
    ttk::frame .vTcl.stat -relief flat
    ...
    $vTcl(gui,rc_menu) add cascade -label "Widget" -menu $vTcl(gui,rc_widget_menu)
    ...
    $vTcl(gui,rc_menu) add comm -label "Hide" -command {
        vTcl:hide
    } -accel "Ctrl+H"
    ...
    ttk::frame .vTcl.stat.f -width 150 -height 15
    ttk::frame .vTcl.stat.f.bar
    ...
    ttk::entry .vTcl.widgetname -textvariable vTcl(w,widget)
...
proc vTcl:define_bindings {} {
    ...
    bind Text <KeyRelease>   {
        ...
        # exclude user inserted text widgets from vTcl bindings
        if {(! [string match .vTcl* %W]) || [info exists %W.nosyntax]  } {
            break
        }
    ...
    if {![info exists env(VTCL_HOME)]} {
	    ...
	    switch [file pathtype $home] {
	        ...
	        volumerelative {
		        set curdir [pwd]
		        cd $home
		        set env(VTCL_HOME) [file join [pwd] [file dirname [file join [lrange [file split $home] 1 end]]]]
		        cd $curdir
	        }
	    }
    }
