...
proc kpwidgets::SBands::create { path args } {
	...
	eval ttk::frame $path $maps(:cmd) -class SBands
...
proc kpwidgets::SBands::new_frame { path args } {
	...
	set lbl $bfrm.lbl
	set frm $bfrm.frm
	pack [ ttk::frame $bfrm ] -expand yes -fill x
	set title [ linsert $title end (+) ]
	pack [ label $lbl -text $title -bg #aaaaaa -bd 1 -relief groove -width 36 ] -side top -fill x -expand yes -padx 2
	ttk::frame $frm 
